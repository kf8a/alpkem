...
l_study = Study.find_or_create_by_name(:name => 'Main Site', :prefix => 'T')
...
l_study.treatments.each do |treatment|
  ...
  replicates.each do |replicate|
    1.upto(3) do |f|
      p = Plot.find_or_create_by_name(:name => "#{treatment.name}#{replicate.name}F#{f}", :study => l_study, :treatment => treatment, :replicate => replicate).save
    end
  end
end