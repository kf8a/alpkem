...
describe Parsers::LysimeterParser do
  ...
  describe 'parsing lysimeter files' do
    ...
    it 'should have the right number of analyses' do
      ...
      sample = @samples[0]
      assert sample.measurements.where(analyte_id: nh4.id).size == 1

      sample = @samples[1]
      assert sample.measurements.where(analyte_id: nh4.id).size == 3
    end
    ...
    it 'should have the right nh4 amounts' do
      nh4 = Analyte.find_by(name: 'NH4')
      sample = @samples[0]
      assert sample.measurements.where(analyte_id: nh4.id).first.amount == 0.046
    end

    it 'should have the right no3 amounts' do
      no3 = Analyte.find_by(name: 'NO3')
      sample = @samples[0]
      assert sample.measurements.where(analyte_id: no3.id).first.amount == -0.030
    end
  end
  ...
  describe 'parsing lachat lysimeter files' do
    ...
    it 'should have the right nh4 amounts' do
      nh4 = Analyte.find_by(name: 'NH4')
      sample = @samples[0]
      m = sample.measurements.where(analyte_id: nh4.id).to_a.first
      assert m.amount == 0.0231
      # assert m.filter { |x| x.amount == 0.0231 }.size == 1
    end

    it 'should have the right no3 amounts' do
      no3 = Analyte.find_by(name: 'NO3')
      sample = @samples[0]
      m = sample.measurements.where(analyte_id: no3.id).to_a.first
      assert m.amount == 0.277
    end
  end
end
